...
# Importações oficiais do SDK v4.x da Adobe
...
async def converter_uma_url_async(url: str, pdf_services: PDFServices, client: httpx.AsyncClient) -> bytes:
    headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)"}
    
    resp = await client.get(url, headers=headers, timeout=15.0)
    resp.raise_for_status()

    def _processar_adobe():
    ...
    return await asyncio.to_thread(_processar_adobe)
